...
class Umeng(BaseCrawler):
    def __init__(self):
        ...
        self.table_stat = [
            'dashboard_general', 'dashboard_trend', 'installations',
            'active_users', 'versions_table', 'silent_users', 'launches',
            'retentions', 'frequency', 'duration', 'depth', 'interval',
            'devices_devices_install', 'devices_resolutions_install',
            'devices_versions_install', 'network_access_launches',
            'network_carriers', 'location_cities', 'index_details'
        ]
        self.benchmark_stat = [
            'DayNewUser', 'DayActiveUser', 'DayLaunchCnt', 'AvgDuration',
            'WeekActiveUser', 'WeekActiveRate', 'MonthActiveUser',
            'MonthActiveRate'
        ]
        self.chart_stat = [
            'error_count', 'error_rate', 'error_affect', 'error_affect_rate',
            'trend_new_users', 'trend_active_users', 'trend_new_users_rate',
            'trend_avg_duration', 'trend_daily_avg_duration',
            'trend_daily_avg_launches', 'trend_avg_traffic_upload',
            'trend_avg_traffic_download', 'index_hours_install',
            'versions_trend_active_user', 'versions_trend_launch', 'freshness',
            'engagement'
        ]
        self.time_unit = [
            'daily',
        ...
        self.stat_type = ['daily_per_launch', 'daily', 'weekly']
        self.message_type = ['alien', 'legit']
        self.extend_stat = ['get_active_user_summary',
                            'get_silent_user_summary']
    ...
    def get_token(self):
        ...
        match = re.search(r'sessionid: \'(\w+)\',', r.text, re.M | re.I | re.S)
        match2 = re.search(r'appkey: \'(\w+)\',', r.text, re.M | re.I | re.S)
        self.token = matchObj.group(1)
        self.sessionid = match.group(1)
        self.appkey = match2.group(1)
        return (self.token ,self.sessionid,self.appkey)

    def login(self):
        if self.is_login is True:
            return True
        token,sessionid,appkey = self.get_token()
        url = 'https://i.umeng.com/login/ajax_do'
        headers = {
            'Accept': '*/*',
            'Accept-Language': 'zh-CN,zh;q=0.8',
            'Cache-Control': 'max-age=0',
            ...
            'referer': 'https://i.umeng.com/?spm=0.0.0.0.egRJTG&null',
            'user-agent':
            'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.110 Safari/537.36',
            'x-requested-with': 'XMLHttpRequest'
        }
        ...
        status_code = json.loads(r.text)['ret']
        if status_code == 200:
            self.is_login = True
        return r.text
    ...
    def gen_other_url(self):
        ...
        for app, item in self.apps:
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/reports/load_table_data?stats=dashboard_general')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/reports/load_table_data?stats=dashboard_trend')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/channels/load_table_data?page=1&per_page=' +
                             self.pagesize + '&stats=realtime&start_date=' +
                             self.start_date + '&end_date=' + self.end_date +
                             '&counts=1')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/channels/load_table_data?page=1&per_page=' +
                             self.pagesize +
                             '&date=today&stats=list&group_id=')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/wau/get_index_table?stats=wau')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/wau/get_transition_raw_data.json?date=' +
                             self.start_date)
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/events/load_table_data?page=1&per_page=' +
                             self.pagesize +
                             '&versions%5B%5D=&stats=count&show_all=false')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/funnels/load_table_data?page=1&per_page=' +
                             self.pagesize + '&period=' + self.start_date +
                             '+-+' + self.end_date +
                             '&time_unit=weekly&stats=funnels')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/error_types/search?start_date=' +
                             self.start_date + '&end_date=' + self.end_date +
                             '&message_type=legit&per_page=' + self.pagesize +
                             '&page=0&order_by=desc_error_count')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/error_types/search?start_date=' +
                             self.start_date + '&end_date=' + self.end_date +
                             '&message_type=alien&per_page=' + self.pagesize +
                             '&page=0&order_by=desc_error_count')
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/reports/load_chart_data?metric=install&stats=index_versions')
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/reports/load_chart_data?metric=install&stats=index_channels')
            other_url.append('http://mobile.umeng.com/apps/' + app +
                             '/wau/get_ratio_trend_chart?start_date=' +
                             self.start_date + '&end_date=' + self.end_date +
                             '&stats=active')
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/channels/load_chart_data?start_date=' + self.start_date +
                '&end_date=' + self.end_date +
                '&versions%5B%5D=&channels%5B%5D=&segments%5B%5D=&time_unit=daily&stats=active_user')
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/channels/load_chart_data?start_date=' + self.start_date +
                '&end_date=' + self.end_date +
                '&versions%5B%5D=&channels%5B%5D=&segments%5B%5D=&time_unit=daily&stats=launch')
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/channels/load_chart_data?start_date=' + self.start_date +
                '&end_date=' + self.end_date +
                '&versions%5B%5D=&channels%5B%5D=&segments%5B%5D=&time_unit=daily&stats=duration')
            other_url.append(
                'http://mobile.umeng.com/apps/' + app +
                '/channels/load_chart_data?start_date=' + self.start_date +
                '&end_date=' + self.end_date +
                '&versions%5B%5D=&channels%5B%5D=&segments%5B%5D=&time_unit=daily&stats=day_retention')
        return other_url
